...
void GPIO2_Combined_16_31_IRQHandler(void)
{
    ...
    static unsigned char _trigger_bytes[4];
    if (GET_GPIO_FLAG(C25))
    {
        _trigger_bytes[2] = 24;
        _trigger_bytes[3] = 231;
        trigger_rec_process(_trigger_bytes);
        CLEAR_GPIO_FLAG(C25);
    }
    if (GET_GPIO_FLAG(C26))
    {
        _trigger_bytes[2] = 28;
        _trigger_bytes[3] = 227;
        trigger_rec_process(_trigger_bytes);
        CLEAR_GPIO_FLAG(C26);
    }
}
...
void GPIO2_Combined_0_15_IRQHandler(void)
{
    ...
    if (GET_GPIO_FLAG(C6))
    {
        ...
        {
            _trigger_bytes[2] = 8;
            _trigger_bytes[3] = 247;
            trigger_rec_process(_trigger_bytes);
        }
        else
        {
            _trigger_bytes[2] = 13;
            _trigger_bytes[3] = 242;
            trigger_rec_process(_trigger_bytes);
        }
        CLEAR_GPIO_FLAG(C6);
    }
